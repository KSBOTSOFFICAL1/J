...
async def _media_local_job(update, context, source: dict, operation: str,
    ...
    try:
        ...
        if operation in ("compress", "compressadvance"):
        ...
        elif operation == "subtitletracksremove":
            cmd = [
                "ffmpeg", "-hide_banner", "-y", "-i", input_path,
                "-map", "0:v?", "-map", "0:a?",
            ...
            await _media_run_ffmpeg(
                update, context, status, source["file_name"], cmd, output_path,
                work_dir, duration, "Removing subtitle tracks...",
                metadata_streams={"video": None, "audio": None, "subtitle": []},
            )
...
async def _media_source_command(update, context, operation: str):
    source = _media_reply_source(update.message.reply_to_message)
    # Also accept a media message whose caption is the command itself. This
    # is useful in groups where users upload a video and add the command in
    # the same message instead of sending a separate reply.
    if not source and operation == "subtitletracksremove":
        source = _media_reply_source(update.message)
    if not source:
        await update.message.reply_text(
...
@require_verification
async def subtitletracksremove_cmd(update, context):
    await _media_source_command(update, context, "subtitletracksremove")
...
def _ensure_event_loop():
    """Keep PTB 21.9 startup compatible with Python 3.14's loop policy."""
    try:
...
def _validate_startup_config():
    """Fail fast with actionable startup diagnostics."""
    problems = []

    if not BOT_TOKEN:
        problems.append(
            "TELEGRAM_BOT_TOKEN is missing. Add it to hosting secrets/environment."
        )
    elif not re.fullmatch(r"\d+:[A-Za-z0-9_-]{20,}", str(BOT_TOKEN).strip()):
        logger.warning(
            "TELEGRAM_BOT_TOKEN is present but has an unusual format; "
            "verify it if Telegram authentication fails."
        )
    ...
    if BOT_OWNER_ID:
        try:
            ...
            problems.append("BOT_OWNER_ID must be a numeric Telegram user ID.")
    else:
        logger.warning(
            "BOT_OWNER_ID is not configured; owner-only features may not work."
        )

    if API_ID and not API_HASH:
        logger.warning("API_ID is set but API_HASH is missing; local Bot API is disabled.")
    elif API_HASH and not API_ID:
        logger.warning("API_HASH is set but API_ID is missing; local Bot API is disabled.")

    if problems:
        for problem in problems:
            logger.error("STARTUP CONFIG ERROR: %s", problem)
        return False

    logger.info(
        "Startup configuration check passed (BOT_TOKEN=%s, BOT_OWNER_ID=%s).",
        "configured",
        "configured" if BOT_OWNER_ID else "missing",
    )
    return True
...
def main():
    ...
    app.add_handler(CommandHandler("subtitletracksremove", subtitletracksremove_cmd))
    subtitle_remove_pattern = re.compile(
        r"^/SubtitleTracksRemove(?:@\w+)?(?:\s|$)",
        re.IGNORECASE,
    )
    app.add_handler(MessageHandler(
        filters.Regex(subtitle_remove_pattern)
        | filters.CaptionRegex(subtitle_remove_pattern),
        subtitletracksremove_cmd,
    ))
    ...
    commands = [
        ...
        BotCommand("subtitletracksremove", "Remove subtitle tracks from a replied video"),
